...
2. `stages.stage7_qa: in_progress` → `qa-runner` 호출. 전달: target_dir 절대경로, 모드, 프로필들, 외부 스킬 경로, 5단계 시나리오 경로.
3. 보고를 받아 `state.yaml` 갱신 (`done`), RR 집계 갱신.
4. 사용자에게: 모드(폴백 여부), 생성/실행/통과/실패 수, 결함·위험 건수, **RR 표**, 중복 제외 건수, 커버리지 공백 요약, 레포트 경로.
