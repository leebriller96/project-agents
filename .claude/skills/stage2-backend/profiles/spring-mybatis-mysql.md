...
- 골격 기본: HikariCP `connection-timeout`, JDBC URL `connectTimeout=5000&socketTimeout=30000`, MyBatis `default-statement-timeout`(초) — DB 응답 정지 시 요청 스레드 무기한 대기 방지. `ErrorController` 를 구현해 컨테이너 오류 디스패치(`/error`, 요청 방화벽 거부)도 `ApiResponse` JSON 으로. `Auditable.markCreated/markUpdated` 는 `Clock` 빈에서 시각을 받는다(`LocalDateTime.now()` 금지).
