...
### B-5. OpenAPI 계약
- slice 오류 코드마다 **사용자 문구**를 계약 description(오류 응답 표)에 싣는다 — 어노테이션은 컴파일 상수만 받으므로 `<Slice>ErrorCode.Msg` 상수 클래스를 두고 enum·서비스·`@ApiResponse` 가 같은 상수를 참조(어긋날 수 없게), 공통 코드 문구는 리플렉션 테스트로 `CommonErrorCode.message()` 와 대조. 오류 응답은 전부 `@Content(schema=@Schema(implementation=ApiResponse.class))` 로 공용 오류 봉투를 가리킨다(성공 봉투 타입 참조 금지 — slice 간 규약 불일치 사례) — FE 가 같은 규칙을 먼저 막을 때 서버와 같은 문구를 쓸 수 있어야 한다(계약만 보고 개발하는 원칙의 필수 조건).
